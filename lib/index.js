...
function Validate(
  SCHEMA,
  OPTIONS = { active: true, verboseErrors: false, logging: false }
) {
    ...
    if (!result.isValid()) {
      if (verboseErrors) {
        if (logging) {
          console.log(
            chalk.red(
              result.errors.reduce((sum, el) => `${sum} \n ${el.message}`, '')
            )
          );
        }
      ...
      if (logging) {
        console.log(chalk.red(JSON.stringify(result.firstError().message)));
      }
